...
{
protected:
  // byte _lightDataBlock.patternID = 0;

public:
  // bool _lightDataBlock.cycleColor = false;

  // byte _lightDataBlock.hue = 0;
  // byte _lightDataBlock.saturation = 0;
  LightDataBlock _lightDataBlock;
  byte _val = 0;

  byte getSelectedPattern() { return _lightDataBlock.patternID; };
  ...
  {
    ioDataBlock->cycleColor = _lightDataBlock.cycleColor;
    ioDataBlock->patternID = _lightDataBlock.patternID;
    ioDataBlock->hue = _lightDataBlock.hue;
    ioDataBlock->saturation = _lightDataBlock.saturation;
  };

  void deserialize(LightDataBlock* iDataBlock)
  {
    _lightDataBlock.cycleColor = iDataBlock->cycleColor;
    _lightDataBlock.patternID = iDataBlock->patternID;
    _lightDataBlock.hue = iDataBlock->hue;
    _lightDataBlock.saturation = iDataBlock->saturation;
  }
};
...
public:
  virtual byte nextPattern() {
    this->_lightDataBlock.patternID = ++(this->_lightDataBlock.patternID) % NUM_LIGHTSTYLES;
  };
  ...
  virtual bool update()
  {
    ...
    if (this->_lightDataBlock.patternID < NUM_LIGHTSTYLES) // don't perform Quake style flicker if we're out of range of those; we'll do Pacifica instead
      this->_val = enhancedQuakeFlicker(this->_lastLightUpdate, this->_lightDataBlock.patternID, this->_prevPatternID, this->_patternStep);
...
public:
  ...
  virtual byte nextPattern()
  {
    this->_lightDataBlock.patternID = ++(this->_lightDataBlock.patternID) % (NUM_LIGHTSTYLES + 1); // add one to support Pacifica as an additional style, which is not handled by the quakeFlicker code
  };
  ...
  bool update()
  {
    ...
    {
      if (_lightDataBlock.patternID < NUM_LIGHTSTYLES)
      {
        byte scaledVal = scale8_video(_val, _maxBrightness);
        setAllLEDs(CHSV(_lightDataBlock.hue, _lightDataBlock.saturation, scaledVal), _leds1, _numLEDs1);
        if (_leds2)
          setAllLEDs(CHSV(_lightDataBlock.hue, _lightDataBlock.saturation, scaledVal), _leds2, _numLEDs2);
        if (_leds3)
          setAllLEDs(CHSV(_lightDataBlock.hue, _lightDataBlock.saturation, scaledVal), _leds3, _numLEDs3);
      }
      else
      {
...
public:
  ...
  virtual bool update()
  {
    if (_lightDataBlock.patternID != _prevPatternID)
    {
      byte distance = patternDistance(_prevPatternID, _lightDataBlock.patternID);
      //Serial.print("updating FL controller, distance between SelectedID (#"); Serial.print(_lightDataBlock.patternID); Serial.print(") and previousID (#"); Serial.print(_prevPatternID); Serial.print(") is "); Serial.println(distance);
      clickFairyLights(distance);
      _prevPatternID = _lightDataBlock.patternID;
      return true;
    }
    return false;
  };
  ...
  virtual byte nextPattern() {
    this->_lightDataBlock.patternID = ++(this->_lightDataBlock.patternID) % _numPatternsAvailable;
  };

  void pulse()
  {
    clickFairyLights(patternDistance(_lightDataBlock.patternID, _onPatternID));
    for (byte i = 0; i < 2; ++i)
    {
      delay(500);
    ...
    clickFairyLights(patternDistance(_onPatternID, _lightDataBlock.patternID));
  };
};
